...
class GithubDeveloperScraper:
    ...
    def upsert_github_user(self, username: str) -> None:
        with self.db_connection.get_session() as session:
            try:
                existing_user = session.query(GithubUserModel).filter_by(username認認username).first()
    ...
    def search_developers(self, language: str) -> Set[str]:
        developers = set()
        try:
            self.driver.get(f"https://github.com/trending/developers/{language}?since=daily")
            follow_buttons = self.driver.find_elements(By.XPATH, '//input[@type="submit" and @name="commit" and @value="Follow"]')
            ...
            raise ScraperException(f"Error searching developers for {language}: {str(e)}")
        return developers
    ...
    def search_followers(self, username: str) -> Set[str]:
        ...
        try:
            ...
            raise ScraperException(f"Error searching followers for {username}: {str(e)}")
        return followers

    def _get_number_of_pages(self, username: str) -> int:
        self.driver.get(f"https://github.com/{username}?tab=followers")
        try:
            span_tag = self.driver.find_element(By.CSS_SELECTOR, 'div.flex-order-1 a.Link--secondary span')
